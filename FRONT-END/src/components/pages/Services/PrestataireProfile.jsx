  ...
  async function getPrestataireById(id) {
    try {
      const response = await fetch(
        `https://festconnectapi.vercel.app/api/providers/${id}`,
        {
          method: "GET",
          headers: {
    ...
    const fetchReviewData = async () => {
      try {
        ...
        const filteredReviews = reviewData.filter(
          (review) => review.prestataireId.id === id
        );
    ...
    <div className={styles.PrestataireProfile}>
          ...
          >
            ...
            <div className="d-flex center h-100">
              <div className="d-flex flex-column center">
                <p>Vous devez être connecté pour pouvoir rédiger un avis.</p>
                <Link to="/login" className="mj-btn-primary">
                  Se connecter
                </Link>
              </div>
            </div>
      ...
      <>
        <div className={styles.btnRetour}>
          <Link className="mj-btn-primary" to="/services">
            {" "}
            <ArrowBackOutlinedIcon /> <p>Retour</p>
          </Link>
        </div>
            ...
            <div className={styles.infosProfile}>
              <div>
                <img
                  ...
                  src={isLiked ? ClickedHeart : NotClickedHeart}
                  alt=""
                />
              </div>
              ...
              <div className={styles.biography}>
                <p>
                  ...
                  <br />
                  {prestataire.location.city}, {prestataire.location.region}
                </p>
              </div>
              <div className={styles.biography}>
                <p>
                  <span>Mobilité </span>
                  <br />
                  Je suis prêt à me déplacer jusqu'à {
                    prestataire.surrounding
                  }{" "}
                  kms{" "}
                </p>
              </div>
        ...
        <div className={`${styles.services} container`}>
          ...
          {matchedSubcategories.map((subcategory) => (
            <p key={subcategory._id}>{subcategory.nameSubCategory}</p>
          ))}
        </div>
        <div className={styles.titlebg}>
          <div className={`${styles.title} container`}>
        ...
        <div className={`${styles.avis} container`}>
            ...
            <div className="container">
              <div className={styles.btnRetour}>
                <button
                  ...
                  className=" d-flex center mj-btn-primary"
                  to="/services"
                >
                  {" "}
                  <ArrowBackOutlinedIcon /> <p>Retour aux avis</p>
                </button>
              </div>
          ...
          <div className="d-flex center">
            {!showReviewForm && (
              <button onClick={handleWriteReview} className="mj-btn-primary">
                Rédiger un avis
              </button>
            )}
          </div>
