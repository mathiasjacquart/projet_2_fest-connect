...
// Upload des photos dans Firebase
  ...
  const handleFileChange = (e) => {
    ...
    const newFiles = files.filter(file => !imgFiles.some(existingFile => existingFile.name === file.name)); // Filtrer les doublons
...
const schema = yup.object({
  ...
  photo: yup.array().of(yup.string()).required("Au moins une photo est requise"),
  service: yup.object({
...
const handleBusinessnameChange = (e) => setSelectedBusinessname(e.target.value);
...
async function onSubmit (values){
  console.log("les données submit ,", values);
  ...
  try {
    ...
    const submissionData = {
      ...
      surrounding: selectedSurrounding,
    };
...
console.log(selectedRegion);
  ...
  const renderForm = () => {
    switch (activeStep) {
                ...
                />
                  ...
                  {imgPreviews.map((link, index) => (
                    <div key={index} className={styles.ButtonFile}>
                      <img src={link} alt={`Aperçu ${index}`} style={{ width: "100%", height: "100%", borderRadius:"10px" }} />
      ...
      <select {...register("service.category")} onChange={handleCategoryChange}>
        <option style={{color:`#8D99AE !important`}} value="">--Sélectionnez un service--</option>
      ...
      {subCategories.length > 0 && (
        <>
          ...
          <select {...register("service.subcategory")}onChange={handleSubCategoryChange}  >
          <option style={{color:`#8D99AE !important`}} value="">--Sélectionnez une catégorie--</option>
          ...
          {errors.service?.subcategories && (
                <p className="text-error">{errors.service?.subcategories.message}</p>
              )}
      ...
      case 3:
        return (
          <div className='d-flex flex-column center'>
            <p style={{fontWeight: "600", marginBottom:"100px", width:"700px",textAlign:"center"}}>Vérifiez que toutes vos informations sont correctes, appuyer sur "Créer son profil" pour finaliser la création de votre profil !</p>
    ...
    <div className={` ${styles.Profile}`}>
      <div className={`container ${styles.ProfileContainer} d-flex flex-column center`}>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          {renderForm()}
          <button style={{marginBottom:"100px"}}type='submit'   onClick={handleSubmit(onSubmit)} className='mj-btn-primary'>Créer son profil</button>
        </form>
